...
[x] An administrator creates and places the desired state of an application into a manifest file.
...
[x] Kubernetes stores the file (an application’s desired state) in a database called the Key-Value Store (etcd).

[x] Kubernetes then implements the desired state on all the relevant applications within the cluster.

[x] Kubernetes continuously monitors the elements of the cluster to make sure the current state of the application does not vary from the desired state.
